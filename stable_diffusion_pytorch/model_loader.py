...
def load_clip(device, dtype=torch.float32):
    state_dict = torch.load(util.get_file_path('ckpt/sd-1.5/clip.pt'))
...
def load_encoder(device, dtype=torch.float32):
    state_dict = torch.load(util.get_file_path('ckpt/sd-1.5/encoder.pt'))
    state_dict = make_compatible(state_dict)
...
def load_decoder(device, dtype=torch.float32):
    state_dict = torch.load(util.get_file_path('ckpt/sd-1.5/decoder.pt'))
    state_dict = make_compatible(state_dict)
...
def load_diffusion(device, dtype=torch.float32):
    state_dict = torch.load(util.get_file_path('ckpt/sd-1.5/diffusion.pt'))
    state_dict = make_compatible(state_dict)
